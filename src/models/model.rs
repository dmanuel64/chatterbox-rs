...
impl Variant<f16> {
    /// Unquantized weights, [`half::f16`] activations.
    pub const FP16: Variant<f16> = Variant::new_inner(Kind::Baseline);
    /// 4-bit-quantized weights, [`half::f16`] activations.
...
impl<F: Float + 'static> Variant<F> {
    ...
    /// feature, since the official graphs are only ever proven to exist as [`f32`] or [`half::f16`]. Use
